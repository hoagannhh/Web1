...
function AddOrUpdateProduct(inforProduct) {
  ...
  if (existingIndex !== -1) {
    ...
    products[existingIndex].quantity = (products[existingIndex].quantity || 1) + 1;
  } else {
...
function RenderProduct(productInfor, container) {
  const productHTML = `
    <div id="${productInfor.id}" class="product-bought">
        <input class="is-bought" type="checkbox" ${productInfor.selected ? 'checked' : ''}>
        <div class="detail-product">
            <div class="represent">
                <img src="${productInfor["img-represent"]}" alt="">
                <div class="navigator">
                    <button class="subtract">-</button>
                    <input class="quantity" type="number" value="${productInfor.quantity || 1}" min="1">
        ...
        <p class="price">${formatVND_manual(productInfor.price * productInfor.quantity)}</p>
    </div>
  `;
...
function RecalculateTotalMoney() {
  ...
  const totalCostElements = document.querySelectorAll(".total-cost, .total-cost-product");
...
function AddEventCheckout() {
  ...
  checkoutBtn.addEventListener("click", () => {
    ...
    sessionStorage.setItem("checkoutProducts", JSON.stringify(productsToCheckout));
